...
CELL_W_OPTIONS: List[int] = [1, 2, 3, 4]
...
def _cell_w() -> int:
    return CELL_W_OPTIONS[CELL_W_IDX]
...
def _build_star_set(t: float) -> dict:
    result = {}
...
def _build_rain_set(DH: int) -> dict:
    result: dict = {}
...
def run_interactive(gen: MazeGenerator, config: object) -> None:
    ...
    def _draw_footer(menu_rows_: list, buf_: str = "") -> None:
        w = sys.stdout.write
        hud_rows = (
    ...
    # ── Full clear + redraw ───────────────────────────────────
    def _full_draw(menu_rows_: list, buf_: str = "") -> None:
        w = sys.stdout.write
    ...
    def _redraw_maze_inplace(maze_lines: list) -> None:
        w = sys.stdout.write
        w("\033[H")
        if not gen.pattern_fits:
            w(
    ...
    # ── One-shot setup before any stepwise animation ──────────
    # Clear screen and draw the footer ONCE before the loop.
    # Inside the loop only maze rows are repainted via cursor-jump
    # — no \033[2J erase, no _full_draw — so the menu never flickers.
    def _prepare_anim(menu_rows_: list) -> None:
    ...
    def _redraw_footer_inplace(menu_rows_: list, footer_row: int) -> None:
    ...
    while True:
        ...
        choice = ""
        live = BG_ANIM or PULSE_WAVE or NEON_CYBERPUNK or RAIN_EFFECT or PLASMA
        if live:
            fd = sys.stdin.fileno()
                ...
                while True:
                    ...
                    if rdy:
                        ...
                        if ch in ("\r", "\n"):
                            ...
                            buf += ch
                            # Choices 2-9: submit on first digit.
                            # Choices 1 / 10-18: first digit is '1',
                            # wait for a second character.
                            if buf[0] != "1":
                                choice = buf.strip()
                                break
                            elif len(buf) >= 2:
        ...
        if choice == "1":
            ...
            sys.stdout.flush()
            try:
                write_output(  # type: ignore[attr-defined]
